...
\caption[Flux relative error by energy group with SPH]{The energy-dependent relative error of the 70-group OpenMOC scalar flux with respect to the reference OpenMC flux in a 1D slab (a) and 2D fuel pin (b) for the innermost, outermost and all \ac{FSR}s.}
...
\label{alg:sph}
\begin{algorithmic}[1]
  \State Initialize \ac{MGXS} from MC tallies
  \State Compute neutron source from MC flux and \ac{MGXS}
  \State Initialize \ac{SPH} factors to unity
  \While{SPH factor residuals are not converged}
    \State Update \ac{MGXS} with \ac{SPH} factors
...
\begin{table}[h!]
  ...
  \toprule
  & \multicolumn{2}{c}{{\bf $\Delta\rho$}} \\
  \cline{2-3}
  \multirow{-2}{*}{{\bf \# Groups}} &
  \multicolumn{1}{c}{{Without SPH}} &
  \multicolumn{1}{c}{{With SPH}} \\
  \midrule
1 & 66 & -14 \\
2 & 34 & -6\\
4 & -57 & 1 \\
...
\end{subfigure}
\caption[Clustering of U-238 capture MGXS standard deviations]{Scatter plots of the pin-wise U-238 capture (group 1 of 2) \ac{MGXS} means ($x$) and standard deviations ($y$) for a 1.6\% enriched fuel assembly.}
...
\label{fig:reflector-geometries}
\end{figure}
...
\label{fig:assm-refl-capt-rates}
\end{figure}
...
\begin{subfigure}{\textwidth}
  ...
  \caption{}
  \label{fig:refl-eigenvalue-converge}
\end{subfigure}
\begin{subfigure}{\textwidth}
  \centering
  \includegraphics[width=0.9\linewidth]{figures/results/convergence/reflector/mean-capt-err-evo}
  \caption{}
  \label{fig:refl-capture-converge-mean}
\end{subfigure}
\vspace{2mm}
\caption[Fission rate covergence for a 2$\times$2 colorset with reflector]{Convergence of the eigenvalue bias (a) and mean absolute U-238 capture rate percent relative errors (b) for a 2$\times$2 colorset with a water reflector.}
