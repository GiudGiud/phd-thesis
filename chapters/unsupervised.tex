...
The $k$-means iteration proceeds until the cluster centroids stop moving as defined by some criterion, such as the residual sum of squares:
...
\begin{algorithmic}[1]
  \State \textit{Initialize} $M$ centroids $\lbrace \boldsymbol{\mu}_{1}, \boldsymbol{\mu}_{2}, \dots, \boldsymbol{\mu}_{M} \rbrace$
  \While{centroid residual is not converged}
    \State \textit{Assign} each sample $\boldsymbol{\hat{f}}_{k}$ to nearest centroid $\boldsymbol{\mu}_{m}$ \Comment{Eqn.~\ref{eqn:chap10-kmeans-assign}}
    \State \textit{Update} centroids to mean of samples assigned to each centroid \Comment{Eqn.~\ref{eqn:chap10-kmeans-update}}
