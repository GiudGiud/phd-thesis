...
The Davies-Boulding (DB) index~\cite{davies1979dbindex} is the ratio of intra-cluster to inter-cluster dis-similarity, or equivalently, the ratio of within-cluster scatter to between-cluster separation. The DB index penalizes cluster models with high \textit{intra-cluster distances} (\textit{i.e.}, low intra-cluster similarities) and low \textit{inter-cluster distances} (\textit{i.e.}, high inter-cluster similarities). The inter-custer distance $d\left(\boldsymbol{\mu}_{m},\boldsymbol{\mu}_{n}\right)$ is simply the distance between cluster centroids $\boldsymbol{\mu}_{m}$ and $\boldsymbol{\mu}_{n}$ in feature space. The most common metric for the intra-cluster distance $R_{m}$ -- and the one used in this thesis -- is the average distance of all samples in cluster $\mathbb{S}_{m}$ to its centroid $\boldsymbol{\mu}_{m}$:
...
The ``best'' clustering model is the one which \textit{maximizes} the Dunn index. The Dunn index is used to quantify clustering models produced by \textit{i}\ac{MGXS} in the following chapter.
