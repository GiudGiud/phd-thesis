...
\end{subfigure}
\caption[U-238 capture rate absolute errors for the quarter core \ac{BEAVRS} model]{U-238 capture rate absolute errors for the quarter core \ac{BEAVRS} model with null (a) and degenerate (b) homogenization corresponding to the reference in Fig.~\ref{fig:chap7-capt-rates-full-core}.}
...
first paragraph: 
-complements Sec.~\ref{sec:chap10-cluster-geometries}

second paragraph: outline
-Sec.~\ref{sec:clustering-without-features}
-Sec.~\ref{sec:clustering-with-dimension-reduce}
...
-highlight the importance of using features
  -rather than simply clustering the MGXS themselves
  -in each case, only cluster MGXS and uncertainties
  -without fractional reactivity, spectral indices, nuclide fractions
-litmus-only feature selection
  -apply litmus tests to determine which nuclides and reaction types to cluster
-no dimensionality reduction
-agglomerative clustering algorithm (Sec.~\ref{subsec:chap10-agglomerative})

second paragraph: outline
-Figs.~\Crefrange{fig:no-features-assm-16-geometries}{fig:no-features-reflector-geometries}
  -individual fuel assemblies and 2$\times$2 colorsets
-Figs.~\Crefrange{fig:no-features-full-core-geometries-8}{fig:no-features-full-core-geometries-64}
  -quarter core models with 2 -- 8 and 16 -- 64 clusters (in powers of 2)

third paragraph: insights
-cluster models are ``noisier'' than those with features in Sec.~\ref{sec:chap10-cluster-geometries}
-illustrates importance of using additional features which may be easier to cluster than the MGXS
  -esp. energy-integrated and/or nuclide-summed

REFERENCE section on clustering without features in Chap. 11
...
\end{subfigure}
\caption[Materials for the 3.1\% assembly without features]{Materials for the 3.1\% enriched fuel assembly with 20 \acp{BP} with \textit{i}\ac{MGXS} homogenization without non-\ac{MGXS} features. The materials for 2, 4, and 8 clusters are illustrated in (a), (c) and (e) for pinch feature selection, and in (b), (d) and (f) for litmus-only feature selection, respectively.}
...
\end{subfigure}
\caption[Materials for the 2$\times$2 colorset with a reflector without features]{Materials for the 2$\times$ colorset with water reflector with \textit{i}\ac{MGXS} homogenization without non-\ac{MGXS} features. The materials for 2, 4, and 8 clusters are illustrated in (a), (c) and (e) for pinch feature selection, and in (b), (d) and (f) for litmus-only feature selection, respectively.}
...
\end{subfigure}
\caption[Materials for BEAVRS without features (2 -- 8 clusters)]{Materials for the quarter core \ac{BEAVRS} model with \textit{i}\ac{MGXS} homogenization without non-\ac{MGXS} features. The materials for 2, 4, and 8 clusters are illustrated in (a), (c) and (e) for pinch feature selection, and in (b), (d) and (f) for litmus-only feature selection, respectively.}
...
\end{subfigure}
\caption[Materials for BEAVRS without features (16 -- 64 clusters)]{Materials for the quarter core \ac{BEAVRS} model with \textit{i}\ac{MGXS} homogenization without non-\ac{MGXS} features. The materials for 16, 32 and 64 clusters are illustrated in (a), (c) and (e) for pinch feature selection, and in (b), (d) and (f) for litmus-only feature selection, respectively.}
...
first paragraph: objective
-highlight impact of using dimensionality reduction (Sec.~\ref{sec:chap10-dimension-reduce})
  -PCA (Sec.~\ref{subsec:chap10-pca})
  -ICA (Sec.~\ref{subsec:chap10-pca})
  -FA (Sec.~\ref{subsec:chap10-factor-analysis})
-litmus-only feature selection
  -with all features, including fractional reactivities, spectral indices and nuclide fractions
  -apply litmus tests to determine which nuclides and reaction types to cluster
-dimensionality reduction
  -PCA, ICA and FA used to reduce features down to just 2 linear combinations of original features
  -applied clustering algorithm within this 2-dimensional space
-agglomerative clustering algorithm (Sec.~\ref{subsec:chap10-agglomerative})
  -with 4 clusters

second paragraph: outline
-Figs.~\Crefrange{fig:dimension-reduce-assm-16-geometries}{fig:dimension-reduce-refl-geometries}
  -individual fuel assemblies and 2$\times$2 colorsets

third paragraph: insights
-different dimensionality reduction techniques lead to very different clustering models
  -PCA is maxmizes variance - tries to find clusters that are as far apart as possible in feature space
  -ICA and FA...who knows?
-author's opinion that dimensionality reduction may be helpful to cluster ``noisy'' MGXS data
-beyond the scope of this thesis to determine reason behind different cluster models
...
REFERENCE section on clustering with dimensionality reduction in Chap. 11
...
\end{subfigure}
\caption[Materials for the 1.6\% assembly with dimensionality reduction]{Materials for the 1.6\% enriched fuel assembly with \textit{i}\ac{MGXS} homogenization with dimensionality reduction. The materials without dimensionality reduction (a) are compared to those identified when PCA (b), ICA (c) and factor analyis (d) are applied, respectively.}
...
\end{subfigure}
\caption[Materials for the 3.1\% assembly with 20 BPs with dimensionality reduction]{Materials for the 3.1\% enriched fuel assembly with 20 \acp{BP} with \textit{i}\ac{MGXS} homogenization with dimensionality reduction. The materials without dimensionality reduction (a) are compared to those identified when PCA (b), ICA (c) and factor analyis (d) are applied, respectively.}
...
\end{subfigure}
\caption[Materials for the 2$\times$2 colorset with dimensionality reduction]{Materials for the 2$\times$2 colorset with \textit{i}\ac{MGXS} homogenization with dimensionality reduction. The materials without dimensionality reduction (a) are compared to those identified when PCA (b), ICA (c) and factor analyis (d) are applied, respectively.}
...
\end{subfigure}
\caption[Materials for the 2$\times$2 colorset with reflector with dimensionality reduction]{Materials for the 2$\times$2 colorset with a water reflector with \textit{i}\ac{MGXS} homogenization with dimensionality reduction. The materials without dimensionality reduction (a) are compared to those identified when PCA (b), ICA (c) and factor analyis (d) are applied, respectively.}
