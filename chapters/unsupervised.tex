...
\begin{algorithmic}[1]
  \State Initialize means $\lbrace \boldsymbol{\mu}_{1}^{(0)}, \boldsymbol{\mu}_{2}^{(0)}, \dots, \boldsymbol{\mu}_{m}^{(0)} \rbrace$ with $k$-Means \Comment{Alg.~\ref{alg:chap10-kmeans}}
  \State Initialize covariances $\lbrace \boldsymbol{\Sigma}_{1}^{(1)}, \boldsymbol{\Sigma}_{2}^{(1)}, \dots, \boldsymbol{\Sigma}_{m}^{(1)} \rbrace$ \Comment{Eqn.~\ref{alg:chap10-em-covariances}}  
  \State Initialize mixture weights $\lbrace \pi_{1}^{(1)}, \pi_{2}^{(1)}, \dots, \pi_{m}^{(1)} \rbrace$ \Comment{Eqn.~\ref{alg:chap10-em-weights}}  
  \While{model parameters are not converged}
    \State Expectation Step: Update posterior probabilities \Comment{Eqn.~\ref{eqn:em-posterior-probs}}
...
-Davies-Boulding (DB) index~\cite{davies1979dbindex}
