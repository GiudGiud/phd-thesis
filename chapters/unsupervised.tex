...
-want to reduce dimensionality of dataset while preserving structure (i.e., relationships between points)
  -make models easier to visualize/explain (same arguments as for feature selection??)
-$P: \mathbb{R}^{J^{*}} \rightarrow \mathbb{R}^{J^{**}}$ where $J^{*} > J^{**}$
...
-Sec.~\ref{subsec:chap10-pca} - PCA
-Sec.~\ref{subsec:chap10-ica} - ICA
-Sec.~\ref{subsec:chap10-factor-analysis} - Factor Analysis
-these are but a few options for dimensionality reduction
  -none of these are used in the following sections, but are briefly mentioned here because of their relevance for future work
...
SUMMARY BOXES
-change feature agglomeration -> dimensionality reduction in figures!!!
...
-PCA provides lower-dimensional picture of the structure in a dataset (reduce dimensionality)
-variance-focused approach which seeks to reproduce total variable variance
  -as compared to factor analysis which is a correlation-focused approach
-orthogonal linear transformation
  -projects set of correlated variables into a linearly uncorrelated orthogonal basis set
  -principal components are linear combinations of the input variables (features)
  -number of principal components is less than or equal to number of original variables
  -principal components are rank-ordered in terms of the amount of variance they account for in the original feature space, with each component orthogonal to all the rest
  -equivalent to the Singular-Value Decomposition (SVD) of the ``data matrix'' ($K \times J^{*}$)
    -data MUST be standardized or the principal components will be sensitive to the mean of each feature

second paragraph: math as a SVD
-$\boldsymbol{\Sigma}$ is a $K \times J^{*}$ rectangular diagonal matrix of positive numbers $\sigma_{(j)}$ known as the singular values (square roots of the eigenvalues)
-$\boldsymbol{U}$ is a $K \times K$ matrix composed of orthogonal vectors called the left singular vectors
-$\boldsymbol{W}$ is a $K \times K$ matrix composed of orthogonal vectors called the right singular vectors
-take the first $L$ largest singular values and their singular vectors as the principal components
-following will transform samples from original feature space to new orthogonal space:
-data matrix is a $K \times J^{*}$ matrix
  -each row is a sample!!!

third paragraph: how do we use it?  
-if each variable in original feature space is i.i.d. ``noisy'' (i.e., white Gaussian noise)
  -PCA helps to increaese the signal-to-noise ratio
  -concentrating the variance in only a few features
  -latter features dominated by the noise
-\texttt{PCA} or \texttt{KernelPCA}~\cite{scholkopf1997kernelpca} from \texttt{sklearn.decomposition}
  -with radial basis function (RBF) kernels to permit project into a non-linear principal component space:
    -kernel is a measure of similarity or affinity between samples
    -may need to normalize sample feature vectors first??
...
\begin{equation}
\label{eqn:chap10-svd-transform}
\boldsymbol{T} = \boldsymbol{U}\boldsymbol{\Sigma} = \boldsymbol{X}\boldsymbol{W} \approx \boldsymbol{X}\boldsymbol{W}_{L}
\end{equation}

\begin{equation}
\label{eqn:chap10-svd-transform-sample}
\boldsymbol{t}_{k} = \boldsymbol{W}^{T}\boldsymbol{\hat{f}}_{k}
\end{equation}
...
first paragraph: what is it?
-~\cite{hyvarinen2000ica}
-often used in signal processing to separate signals into additive subcomponents
-assumes 1) subcomponents are non-Gaussian and 2) statistically independent
-ICA finds independent components (aka sources, factors, latent variables)
  -AIMS TO MAXIMIZE STATISTICAL INDEPENDENCE OF THE ESTIMATED COMPONENTS!!!
    -this is different than PCA, which maximizes the remaining sample variance explained by each component

second paragraph: math
-the mixing matrix $\boldsymbol{W}$ transforms the sample input $\boldsymbol{\hat{f}}_{k}$ into a vector $\boldsymbol{s}_{k}$ of the maximally independent subcomponents (or sources)
  -maximal independence is measured by some function of independence, including:
    -Kullback-Leibler (KL) Divergence
    -maximum entropy
    -kurtosis
  -need to estimate both the mixing matrix $\boldsymbol{W}^{-1}$ and the sources $\boldsymbol{s}_{k}$
-noiseless vs. noisy ICA (whether or not data is assumed to be noisy)
-``noisy'' ICA adds zero-mean and uncorrelated Gaussian noise vector $\boldsymbol{\epsilon}$:

third paragraph: how do we use it?
-\texttt{FastICA}~\cite{hyvarinen1999fastica} from the \texttt{sklearn.decomposition} module
  -uses the kurtosis as the ``cost'' function
  -fixed-point iteration scheme
-refer reader to the literature for more information :-)
-need to add option for ICA decomposition to scripts
  -run a few test cases and see what comes out!!!

\begin{equation}
\label{eqn:chap10-ica}
\boldsymbol{s}_{k} = \boldsymbol{W}\boldsymbol{\hat{f}}_{k}
\end{equation}

\begin{equation}
\label{eqn:chap10-ica-inverse}
\boldsymbol{\hat{f}}_{k} = \boldsymbol{W}^{-1}\boldsymbol{s}_{k}
\end{equation}

\begin{equation}
\label{eqn:chap10-ica-inverse-noisy}
\boldsymbol{\hat{f}}_{k} = \boldsymbol{W}^{-1}\boldsymbol{s}_{k} + \boldsymbol{\epsilon}_{k}
\end{equation}
...
first paragraph: what is it?
-~\cite{harman1960factor}
-inspired by applications in the social sciences like psychology
-a probabilistic model with a likelihood function (equivalent to probablistic PCA or PPCA)
-describe variablity among observed correlated variables in terms of latent ``factor'' variables
  -unobserved variables modeled as linear combinations of factors plus error terms
-model the variance in every direction of the input space independently (heteroscedastic noise)
  -better model selection than PCA in the presence of heteroscedastic noise
  -heteroscedastic: variability of a variable is unequal across the range of values of a second variable that predicts it
    -this would be the case for LSVM

second paragraph: math

third paragraph: how do we use it?
-ICA is similar but does *not* specify a gaussian prior over the latent variables
-\texttt{FactorAnalysis} in \texttt{sklearn.decomposition} module
-need to add option for factor analysis decomposition to scripts
  -run a few test cases and see what comes out!!!

\begin{equation}
\label{eqn:chap10-lvm}
\boldsymbol{\hat{f}}_{k} = \boldsymbol{W}\boldsymbol{h}_{k} + \boldsymbol{\mu} + \boldsymbol{\epsilon}
\end{equation}

where $\boldsymbol{h}_{k}$ is ``latent'' and $\boldsymbol{\epsilon}$ is a Gaussian noise term distributed with zero mean and covariance $\boldsymbol{\Sigma}$ or $\boldsymbol{\epsilon} \sim \mathcal{N}\left(0,\boldsymbol{\Sigma}\right)$:
-assume that $\boldsymbol{\Sigma}$ is a diagonal matrix for factor analysis
  -it is also diagonal for PCA with identical values along the diagonal
-$\boldsymbol{W}$ is the ``factor loading matrix''

\begin{equation}
\label{eqn:chap10-lvm-prob}
