...
first paragraph: explain table
-Tab.~\ref{table:chap11-runtimes}
-explain objective of this section
-Tab.~\ref{table:chap11-openmc-rates} - particle tracking rates
  -particle tracking rates; dependent on hardware platform and runtime configuration
-Tab.~\ref{table:chap11-runtimes} - runtimes
-10 clusters for assemblies and periodic colorset; 16 clusters for coloreset with reflector
-refer to data from Sec.~\ref{sec:chap11-converge} to determine convergence
  -conservative analytical approach:
    1) determine the  converged mean error for \textit{i}\ac{MGXS} scheme for each benchmark
      -mean error chosen since it is more limiting for OpenMC
    2) determine the approx. number of histories to reach the converged accuracy for each scheme
      -null's accuracy converged to a bias greater than \textit{i}\ac{MGXS}
      -\textit{i}\ac{MGXS} and degenerate convergence to prescribed accuracy determined by:
        -analyzing the error at each of the statepoints (stored at logarithmically spaced intervals)
        -error was considered ``converged'' once it fell below the accuracy criterion for three successive statepoints
        -the number of histories was derived from the third statepoint 
        -the number of histories is highly dependent on the number of clusters used by \textit{i}\ac{MGXS}
      -used same procedure to determine number of histories to converge OpenMC uncertainties
      -round number of particle histories up for simplicity and to be conservative
      -this analysis is limited by the logarithmically-spaced statepoint intervals
    3) use particle tracking rate to compute total time spent by OpenMC
      ...
      -uses the tracking rate without MGXS for the ``reference''
      -use the tracking rate with MGXS for all others
    4) record OpenMOC runtime for each benchmark and scheme
      -note that the runtimes for the degenerate scheme were on average 20\% greater than those for the
  -this analysis neglects the time spent processing the tally data to create the \ac{MGXS} libraries since this was negligible (\textit{i.e.}, <0.1 core-hour) for all benchmarks and configurations of the \textit{i}\ac{MGXS} data processing pipeline that were tested)

third paragraph: observations
-speed to reach the desired accuracy:
-null is by far the fastest approach, but suffers from the largest error
...
& \textit{i}\ac{MGXS}\footnotemark & 0.10 & 65,000,000 & 15 & 0.4 & 15 \\
...
& \textit{i}\ac{MGXS}\textsuperscript{\ref{birch-assm}} & 0.10 & 65,000,000 & 15 & 0.4 & 15 \\
& Degenerate & 0.10 & 500,000,000 & 126 & 0.4 & 126 \\
& Reference & 0.10 & 650,000,000 & 60 & & 60 \\
  \midrule
\multirow{4}{*}{\parbox{2.5cm}{3.1\% Assm w/ 20 \acp{BP}}} & Null & 0.41 & 100,000 & 0.025 & 0.4 & 0.42 \\
& \textit{i}\ac{MGXS}\textsuperscript{\ref{birch-assm}} & 0.13 & 65,000,000 & 15 & 0.4 & 15 \\
& Degenerate & 0.13 & 325,000,000 & 82 & 0.4 & 82 \\
& Reference & 0.16 & 450,000,000 & 42 & & 42 \\
  \midrule
\multirow{4}{*}{\parbox{2.5cm}{2$\times$2 Colorset}} & Null & 0.45 & 100,000 & 0.035 & 2.0 & 2.0 \\
& \textit{i}\ac{MGXS}\textsuperscript{\ref{birch-assm}} & 0.16 & 100,000,000 & 35 & 2.0 & 35 \\
...
& \textit{i}\ac{MGXS}\footnotemark & 0.25 & 85,000,000 & 30 & 5.0 & 35 \\
...
& \textit{i}\ac{MGXS}\footnotemark & & & & 423 & \\
& Degenerate & & & & 426 & \\
& Reference & & & & &\\
  \bottomrule
\end{tabular}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Comparison with Multi-Level Approach}
