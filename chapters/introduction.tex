...
Core simulation tools utilizing \ac{MC} transport methods represent the ``Holy Grail'' for reactor analysis. However, the computational costs required to accommodate the slow convergence rate for \ac{MC} makes it impractical for widespread use for the foreseeable future. Deterministic transport methods pose a number of advantages to \ac{MC} from a computational perspective, which seems likely to make these methods more viable in the near to intermediate term. However, deterministic methods almost universally discretize the energy domain in a few to hundreds of energy groups. This approximation requires energy condensed multi-group cross-sections in each geometric region to preserve reaction rates. One of the key challenges is accurate, \emph{reactor agnostic} multi-group cross-section generation methods to enable deterministic transport-based methods to be as accurate and flexible as Monte Carlo.

This thesis investigates the potential use of \ac{Monte Carlo} methods to generate \ac{MGXS} for next-generation whole-core deterministic reactor analyis. Monte Carlo presents a natural approach to replace engineering prescriptions to approximate the flux with stochastic approximations of the exact flux obtained from \ac{MC}. \ac{MC} methods have been increasingly been used to generate few group constants for coarse mesh diffusion, most notably by the Serpent \ac{MC} code [CITE]. However, there exist few rigorous and comprehensive analyses of \ac{MC} as an approach to generate \ac{MGXS} for heterogeneous fine-mesh transport. The advantage of such approach are numerous in that all of the relevant physics modeled in \ac{MC} may be directly embedded into \ac{MGXS}. However, this potential improvement in accuracy comes at the computational expense of tallying group constants with acceptably low uncertainties.

Such an approach - benefit, exact; disadvantage, slow to converge

Third paragraph\\
-mention MG would be the reference soln anyway
-embed accuracy of Monte Carlo into MGXS for deterministic transport
-mention need to accelerate it in way not possible for conventional transport
-replace deterministic approximation 
-natural, no approximation to the flux-weighting spectrum
