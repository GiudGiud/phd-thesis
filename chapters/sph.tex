...
Gibson proceeded to show that the angular-dependent \ac{MGXS} eliminated the reaction rate errors observed with scalar flux-weighted \ac{MGXS}. The analysis highlighted the tight coupling between angular-dependence and the spatial discretization. Notably, the reaction rate errors were reduced by an order of magnitude only when angular-dependent \ac{MGXS} were paired with a fine \ac{FSR} spatial discretization. This can be explained by the fact that the angular variation of the volume-integrated flux is moderated as the \ac{FSR} mesh is coarsened. Thus, the angular effects at the fuel/moderator interface cannot be captured by angular-dependent \ac{MGXS} if the \ac{FSR} discretization does distinguish between neutrons entering/leaving the fuel pin.
