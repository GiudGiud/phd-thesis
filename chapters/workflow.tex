...
-first paragraph in LNS section in OpenCG paper
-can we identify symmetries in a reactor core geometry to predict regions with similar physics?
-analogy to lattice physics geometric templates based on eng. approx.
-identify/predict regions which may experience similar spectral self-shielding effects
  -heuristic to identify fuel pins
-development and implementation of LNS algorithm
-this algorithm was explicitly developed for this thesis
-no other implementation known to author

second paragraph: algorithmic description
-second paragraph in \ac{LNS} section in OpenCG paper
Breadth First Search at k levels of CG tree to construct $k$-partite graph
-$k$-partite graph, hash, etc.
-reference algorithm
-reference k-partite graph figure
-more details about algorithm can be found in the paper

third paragraph: general vs. unique neighbors
-combine paragraphs 3-4 in LNS section in OpenCG paper
-reference figure with fuel assemblies
  -extract the caption from the figure?

fourth paragraph: tie back to MGXS
-mention that this is used as a deterministic ``clustering'' technique in spatial homogenization methodology
-analogy to lattice physics geometric templates based on eng. approx.
...
\begin{figure}[h!]
  ...
  \caption[OpenCG region differentiation process diagram]{The OpenCG \ac{LNS} algorithm may be used to to generate region groupings for the region differentiation algorithm to create an input geometry for OpenMC or OpenMOC.}
