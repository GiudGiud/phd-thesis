...
\vspace{2mm}
\caption[A 2$\times$2 colorset of BEAVRS assemblies with a reflector]{A 2$\times$2 colorset of BEAVRS assemblies surrounded by a water reflector. Reflective \acp{BC} were applied on the left/top, with vacuum \acp{BC} on the right/bottom.}
...
\vspace{2mm}
\caption[The 2D quarter core BEAVRS model]{The 2D quarter core \ac{BEAVRS} model.}
...
\begin{table}[h!]
  \centering
  \caption[Reference OpenMC eigenvalues for each benchmark]{Reference $k^{OpenMC}_{eff}$ for heterogeneous benchmarks.}
  \small
  ...
  \ac{BEAVRS} Quarter Core & 1.02446 $\pm$ 0.00001 & 10,892 \\  % $\pm$ 0.00000
  \bottomrule
\end{tabular}
\end{table}
...
\end{subfigure}%
\caption[Fission rates for BEAVRS]{Fission rates for the 2D quarter core \ac{BEAVRS} model.}
...
\end{subfigure}%
\caption[U-238 capture rates for BEAVRS]{U-238 capture rates for the 2D quarter core \ac{BEAVRS} model.}
