...
num_sps = 5
statepoints = np.logspace(np.log10(inactive+10), np.log10(batches), num_sps, dtype=np.int)
#former = np.logspace(np.log10(10), np.log10(1000), num_sps * 0.8, dtype=np.int) + inactive
#latter = np.logspace(np.log10(1201), np.log10(batches), num_sps * 0.2, dtype=np.int)
#statepoints = np.unique(np.concatenate((former, latter)))
