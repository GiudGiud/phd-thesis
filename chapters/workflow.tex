...
-see first paragraph in RD section in OpenCG paper
  -perhaps take paragraph verbatim
-retain CSG's simplicity of repeated primitives for user input
-build combinatorial geometries based on arbitrary clustering of cell instances
-motivate why this is useful for MGXS 
  -How can we update a combinatorial geometry model to reflect local neighbor symmetries?

second paragraph: region differentiation scheme as a soln
-second paragraph in RD section in OpenCG paper
-time consuming and error prone to build CG
-systematic and automated construction with region differentiation
-reference figure

-SEE SLIDE 60
-The OpenCG API takes region groupings as input (cell instances to treat as identical)
-Dynamic programming algorithm climbs one level of the CG tree per iteration
-At each level, the paths for each region in each grouping is hashed to find primitive collisions
-All primitive collisions are resolved through primitive differentiation
-Terminates when no two regions have an updated path which results in a primitive collision

third paragraph: region groupings 
-third paragraph in OpenCG paper
-define region groupings: all regions that reference a particular cell in a geometry
-tie back to figure

fourth paragraph: dynamic programming to avoid factorial scaling
-fourth paragraph in OpenCG paper

fifth paragraph: intrinsic collisions
-fifth paragraph in OpenCG paper

sixth paragraph: tie back to MGXS
-figure on slide 50

-can any of these paragraphs be combined??
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=\linewidth]{figures/workflow/opencg/region-diff-proc-diagram}
