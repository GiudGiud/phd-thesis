...
-need to evaluate and assess clusterings and select the ``best'' one
  -aka cluster validation???
-need criterion to quantify ``quality'' of a clustering and/or to compare similarity between clusterings
-must be quantifiable so it can be automated
  -a human cannot be required to ``inspect'' clustering
  -this isn't doable since visualizations in high-dimensional feature space aren't possible anyway
-need to consider and present concept of \textit{over-fitting}
-need to strike a balance when choosing the number of clusters between:
  -maximum compression of data with a single cluster
  -maximum accuracy by assigning each data point to its own cluster
-want to choose optimal number of clusters so that adding another cluster doesn't model the data much better
  -in our case, ``better'' is determined by the quality of the OpenMOC solution
  -first few clusters will ``explain'' most of the variance of the data
  -the addition of more cluters will add marginal information
-Fig.~\ref{fig:chap10-model}
...
second paragraph: types of methods:
  -External Evaluation: evaluated with some labeled data that was left of training phase
    -requires some ground truth cluster labels for some or all samples
    -this is not the case for truly unsupervised applications like \textit{i}\ac{MGXS}
    -external evaluation metrics compare predictions of cluster model to the true labels
  -Interal Evaluation: evaluate based on the data that was used to train the predictor
    -high similarity within a cluster
    -low similarity between clusters
    -i.e., the same as our rough definition of a ``cluster''
...
\label{sec:chap10-calinski-harabaz}
...
\label{sec:chap10-cross-validate}
