...
Up to this point the \ac{MGXS} have been defined as continuously varying in space. Most deterministic methods for solving the multi-group transport equation discretize space and assume that the material properties are constant across each mesh cell. \textit{Spatial homogenization} is used to to compute volume-averaged cross sections within each mesh cell $i$ with volume $V_{i}$ are defined as:
