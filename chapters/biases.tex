...
As illustrated in the table, both U-238 capture and total absorption rate error grows with the number of energy groups used in the \ac{MGXS} calculation. This trend is particularly pronounced for U-238 capture in the 6.67 eV resonance group which exceeds 1\% for 25 or more groups. The 0.35 -- 0.37\% error in the total absorption would directly correspond to an under-prediction in the eigenvalue of 350 -- 370 pcm. This compares closely with the observed 300 -- 350 pcm eigenvalue bias, even without accounting for the error in the multi-group fission rates. These results indicate that spatial self-shielding effects in resonance groups is not adequately captured by the \ac{MGXS} and/or the multi-group calculation as will be discussed in more detail in Chapter~\ref{chap:sph}.

\begin{itemize}[noitemsep]
  \item compute frac of abs in the resonance regime
  \begin{itemize}[noitemsep]
    \item if ~0.3\% then 1\% error in resonance regime drives eigenvalue error
  \end{itemize}
  \item compute frac of abs in group 27
  \begin{itemize}[noitemsep]
    \item if ~0.3\% then 1\% error in group 27 drives eigenvalue error
  \end{itemize}
\end{itemize}
...
%\begin{table}[h!]
%  \centering
%  \caption[U-238 capture to total absorption for a 1D slab.]{The percentage of U-238 capture to total absorption in a 1D slab.}
%  \small
%  \label{table:chap4-slab-capture-frac} 
%  \vspace{6pt}
%  \begin{tabular}{c c c c}
%  \toprule
%  \multicolumn{1}{c}{\bf \# Groups} &
%  \multicolumn{1}{c}{\bf Group 27} &
%  \multicolumn{1}{c}{\bf Groups 14 -- 27} &
...
%4 & 81.17 & 81.17 & 46.31 \\
%8 & 81.17 & 81.17 & 46.32 \\
%16 & 81.17 & 81.17 & 46.32 \\
%25 & 77.30 & 68.92 & 46.39 \\
%40 & 77.30 & 68.92 & 46.40 \\
%70 & 77.30 & 67.94 & 46.40 \\
%  \bottomrule
%\end{tabular}
%\end{table}
...
%  \label{table:chap4-pin-isotopes} 
%  \vspace{6pt}
%  \begin{tabular}{l c c}
%  \toprule
%  \multicolumn{1}{c}{\bf Material} &
...
Table~\ref{table:chap4-pin-energy} presents the bias $\Delta\rho$ between OpenMC and OpenMOC for a matrix of energy group structures and \ac{FSR} spatial discretizations. In each case, the \ac{MGXS} used in OpenMOC were tallied by material in OpenMC. The OpenMOC calculations each used 128 azimuthal angles and 0.05 cm track spacing. Each of the materials in the fuel pin was radially discretized into 1 -- 16 equal volume rings. 
