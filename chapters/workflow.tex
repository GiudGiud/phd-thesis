...
first paragraph: need tool to tied together OpenMC/MOC
-first paragraph from OpenCG paper
  -tailor to this purpose, use bullets
-both openmoc, openmc use combinatorial geometry
-enables one to build single geometry and export to OpenMC/MOC
-facilitates large scale data processing on a combinatorial geometry mesh
-simple Python interface
-thesis developed tool
-reference simulation triad figure
-OpenCG paper~\cite{boyd2015opencg}

second paragraph: mesh transfer is key reason for code
-novel algorithms to support novel spatial homogenization methods
-``glue'' code to map OpenMC distribcells to OpenMOC spatial mesh
-need mesh transfer functionality:
  -OpenMC is "mesh-less"
  -OpenMOC requires mesh discretization
  -MGXS data from OpenMC mesh to OpenMOC
  -rxn rate comparisons between OpenMC and OpenMOC
  -map OpenMC distribcells to OpenMOC FSRs
    -OpenCG includes variant of distribcell algorithm in OpenMC

third paragraph: overview of sections
-thesis developed the tool
-compatibility modules
  -single physics-agnostic geometries~\cite{boyd2015opencg}
-LNS scheme
-region differentiation scheme
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=.8\linewidth]{figures/workflow/opencg/neighbors-k-partite-graph}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=.8\linewidth]{figures/workflow/opencg/region-diff-proc-diagram}
