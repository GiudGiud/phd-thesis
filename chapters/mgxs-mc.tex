...
-serpent\\
-McCARD~\cite{park2012generation} developed at Seoul National University, capable of generationg \ac{MGXS} for diffusion theory codes\\
-MCNP~\cite{cho2009generation} homogenized nodal parameters with albedo-corrected leakage spectrum\\
-MCNP~\cite{van2006homogenized} collision estimator for homogenized \ac{MGXS} but limited to only cases with reflective boundary conditions\\
-TRIPOLI-4~\cite{both1997computations} first development work to generate \ac{MGXS} with TRIPOLI-4 code for validation \ac{MGXS} calculated with sub-assembly codes and examine spatial self-shielding models (again for diffusion theory codes)\\
