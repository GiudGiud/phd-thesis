...
first paragraph:
-try to find patterns between like data points
-all features treated on an equal basis

-define each clustering algorithm as minimizing some metric
  -k-Means: inertia
  -

-define assumptions for each clustering algorithm:
  -

-make it real for this application!!!
...
-introduce scheme~\cite{macqueen1967kmeans, lloyd1982kmeans}
-intro inertia minimization
-partitions of feature space into Voronoi cells
-set of $C$ disjoint clusters
-not to confuse $k$ for the fuel pin instance index!!!
-minimize inertia\footnote{The inertia is a within-cluster sum-of-squares criterion.}:

\begin{equation}
\label{eqn:chap10-kmeans-inertia}
\displaystyle\sum\limits_{k=0}^{K} \min_{\boldsymbol{\mu}_{j} \in C} \|\boldsymbol{\hat{f}}_{k} - \boldsymbol{\mu}_{j}\|^{2}
\end{equation}

second paragraph: algorithm
-Alg.~\ref{alg:chap10-kmeans}
-step 1 (initialization): choose initial centroids
-loop until centroids stop moving:
  -step 2 (assignment): assign each sample to nearest centroid
  -step 3 (update): create new centroids from mean of those points assigned to each centroid
...
-\texttt{KMeans} of the \texttt{sklearn.cluster}
-fast and scalable to datasets with large number of samples $K$ and features $J$
  -requires distance calculation from for each data point to each cluster centroid
  -easy to parallelize, for large datasets with out-of-core mini-batch $k$-Means algorithm~\cite{sculley2010minibatchkmeans} implemenbted by \texttt{MiniBatchKMeans}

fourth paragraph: assumptions and guarantees
-inertia assumes clusters are convex and isotropic
  -actually assumes clusters are spherical in feature space
  -assumes clusters are of the similar size
  -not well suite to clusters with irregular shapes
-provably guaranteed to converge if Euclidean distance is used
  -which is the case for this thesis
-requires the number of clusters to be defined
  -need model selection to determine $k$
-$k$-Means will always converge, but to a local minimum of the inertia
  -partioning is highly dependent on the initialization of centroids
  -can randomly choose $k$ data points as initial centroids
  -$k$-Means++~\cite{arthur2007kmeans++} 
    -\texttt{init=kmeans++} argument to \texttt{KMeans}
    -attempts to uniformly disperse centers across regions of feature space with samples
    -this is what is used in this thesis
  -since algorithm is very fast, run many times with different initial centroids and choose solution with smallest inertia
-computational complexity of $\mathcal{O}(KJ^{*}MT)$ for $K$ fuel pin instances, $J^{*}$ features, $M$ clusters and $T$ iterations

\begin{equation}
\label{eqn:chap10-kmeans-assign}
\mathbb{S}_{m}^{(t)} = \lbrace \boldsymbol{\hat{f}}_{k} \;\; : \;\; \| \boldsymbol{\hat{f}}_{k} - \boldsymbol{\mu}_{m}^{(t)} \|^{2} \;\; \le \;\; \| \boldsymbol{\hat{f}}_{p} - \boldsymbol{\mu}_{j}^{(t)} \|^{2} \;\;\;\;\;\;\; \forall j, \; 1 \; \le j \le M \rbrace
\end{equation}
...
res = \max \|\boldsymbol{\mu}_{m}^{(t+1)} - \boldsymbol{\mu}_{m}^{(t)}\|^{2}
\end{equation}
...
\begin{algorithmic}[1]
  ...
  \EndWhile
%  \State Compute final inertia of cluster centroids \Comment{Eqn.~\ref{eqn:chap10-kmeans-inertia}}
\end{algorithmic}
\end{algorithm}
...
first paragraph: intro
-introduce scheme~\cite{johnson1967hierarchical}
-greedy algorithm - merges and splits greedily at each step
-minimize linkage criterion at each step
-build clusters through recursive splitting or merging
  -agglomerative clustering in \texttt{scikit-learn} uses a bottom-up approach
-clusters are built as a dendogram

second paragraph: algorithm
-Alg.~\ref{alg:chap10-agglomerative}
-affinity/distance:
  -euclidean distance $\ell_{2}$ (used in this thesis)
  -manhattan distance $\ell_{1}$
linkage criteria: various cluster ``dissimiliarity'' metrics that are minimized *at each step*
  -Complete: minimize max distance between pairs of points within two clusters to be merged
  -Average: minimize avg. of dist. between pairs of points within two clusters to be merged
 -Ward: minimize sum of squared differences between points in two clusters to be merged~\cite{ward1963hierarchical}
  -variance minimization, similar to $k$-Means
  -used by this thesis
...
third paragraph: implementation
-\texttt{AgglomerativeClustering} of \texttt{sklearn.cluster} module

fourth paragraph: assumptions and guarantees
-choice of linkage criterion impacts shape of clusters
-connectivity constraints
  -forbid merging clusters which are not adjacent, and impose some local structure
  -could use $k$-nearest neighbors graph to ensure that only clusters with MGXS adjacent to one another are merged
-must specify number of clusters
  -requires model selection
-computational complexity is $\mathcal{O}(n^{2}\log(n))$ -- too slow for large datasets
-``rich get richer'' behavior with uneven cluster sizes
  -though depends on linkage criteria
-no assumptions about shape or size of clusters

%\begin{equation}
%\label{eqn:chap10-agglomerative-ward}
%d_{a,b} = d\left(\boldsymbol{\hat{f}}_{a}, \boldsymbol{\hat{f}}_{b}\right) = \|\boldsymbol{\hat{f}}_{a} - \boldsymbol{\hat{f}}_{b}\|^{2}
%\end{equation}
...
\begin{algorithmic}[1]
  ...
  \While{there are too many clusters: $|\mathbb{C}| > M$}
    \State Compute pairwise distances between clusters \Comment{Eqn.~\ref{eqn:chap10-agglomerative-ward}}
...
first paragraph: intro
-introduce scheme~\cite{zhang1996birch}
-a hierarchical clustering algorithm
-Balanced Iterative Reducing and Clustering Using Hierarchies (BIRCH)
-designed as an out-of-core clustering algorithm for very large datasets (in number of samples)
-considered a data reduction method: reduces dataset to subclusters, which are then clustered!
-does not require inspecting all data points equally for each ``clustering decision'' as does $k$-Means or agglomerative clustering
  -``local'' use CF of subclusters to make ``clustering decisions''

second paragraph: algorithm
-Alg.~\ref{alg:chap10-birch}
-builds a Characteristic Feature Tree (CFT)
  -height-balanced tree governed by a branching factor $B$ and threshold $T$
  -each non-leaf node contains a CF and a pointer to a child node
  -each leaf node contains a CF
  -all entries in a leaf node must have a radius less than the threshold
-Characteristic Features (CF)
-two parameters:
  -threshold $T$: limits distance between entering sample and existing subclusters
  -branching factor $B$: limits number of subclusters in a node
-CF additivity theorem~\cite{zhang1996birch} allows two clusters to merged without loss of information
-CF subcluster nodes (terminal and non-terminal)
  -hold information necessary to represent each subcluster without needing to explicitly store each of the data points assigned to the subcluster
    -number of samples, linear sum, squared sum

third paragraph: implementation
-\texttt{Birch} of \texttt{sklearn.cluster} module
-use existing clustering algorithm to cluster leaf nodes
  -\texttt{scikit-learn} uses \texttt{AgglomerativeClustering}

fourth paragraph: assumptions and guarantees
-predicate on assumption that feature space is not uniformly occupied
  -e.g., not every data point is equally important

-cluster centroids $\boldsymbol{\mu}_{\mathbb{N}_{a}}$ computed per Eqn.~\ref{eqn:chap10-kmeans-update}

\begin{equation}
\label{eqn:chap10-birch-radius}
R_{a} = \left(\frac{1}{|\mathbb{N}_{a}|}\displaystyle\sum\limits_{\boldsymbol{\hat{f}}_{k} \in \mathbb{N}_{a}} \left(\boldsymbol{\hat{f}}_{k} - \boldsymbol{\mu}_{a}\right)^{2}\right)^{\frac{1}{2}}
\end{equation}

\begin{equation}
\label{eqn:chap10-birch-linear-sum}
LS_{a} = \displaystyle\sum\limits_{\boldsymbol{\hat{f}}_{k} \in \mathbb{N}_{a}} \boldsymbol{\hat{f}}_{k}
\end{equation}

\begin{equation}
\label{eqn:chap10-birch-square-sum}
SS_{a} = \displaystyle\sum\limits_{\boldsymbol{\hat{f}}_{k} \in \mathbb{N}_{a}} \boldsymbol{\hat{f}}_{k}^{2}
\end{equation}

\begin{equation}
\label{eqn:chap10-birch-cf}
CF_{a} = \left[|\mathbb{N}_{a}|, LS_{a}, SS_{a}\right]^{T}
\end{equation}

\begin{equation}
\label{eqn:chap10-birch-cf-additivity}
CF_{a+b} = CF_{a} + CF_{b} = \left[|\mathbb{N}_{a}| + |\mathbb{N}_{b}|, LS_{a} + LS_{b}, SS_{a} + SS_{b}\right]^{T}
\end{equation}
...
\begin{algorithmic}[1]
  ...
  \For{each sample $k$}
    \State Initialize CFT node: $\mathbb{N} \leftarrow \mathbb{N}_{r}$
    \While{$\mathbb{N}$ is not a leaf node} 
      \State Compute radii for each subcluster if merged with sample \Comment{Eqn.~\ref{eqn:chap10-birch-radius}}
      \State Find subcluster $\mathbb{N}_{a}$ with smallest post-merged radius $R_{a+k}$
	  \State Assign $\mathbb{N} \leftarrow \mathbb{N}_{a}$
    \EndWhile
    \State Add sample to $\mathbb{N}$ and update $CF_{\mathbb{N}}$ \Comment{Eqn.~\ref{eqn:chap10-birch-cf-additivity}}
    \While{$\mathbb{N} \ne \mathbb{N}_{r}$ } 
      \If{$R_{\mathbb{N}} > T$}
        \If{$|\mathbb{N}| > B$}
          \State Split subclusters in $\mathbb{N}$ into $\mathbb{N}_{a}$ and $\mathbb{N}_{b}$
        \EndIf
        \State Assign parent subcluster $\mathbb{N}_{p}$ with sample $k$ to $\mathbb{N}$: $\mathbb{N} \leftarrow \mathbb{N}_{p}$
      \EndIf
   \EndWhile
  \EndFor
  \State Input leaf nodes to agglomerative clustering algorithm \Comment{Alg.~\ref{alg:chap10-agglomerative}}
\end{algorithmic}
\end{algorithm}
...
-a probabilistic or generative model
  -hierarchical
...
\begin{tikzpicture}
  ...
  \node[param] (mu) [above of=pi,yshift=20mm, label=below:$\boldsymbol{\mu}_{m}$] { };
  \node[main, fill = black!10] (f) [right of= ell,label=below:$\boldsymbol{\hat{f}}_{k}$] { };
  \node[param] (sigma) [above of=ell,yshift=20mm, label=below:$\sigma_{m}^2$] { };
...
\label{fig:chap10-gmm}
\end{figure}

-Bayesian Information Criterion (BIC)~\cite{schwarz1978bic} to select number of components
  -should this be reserved for the section on model selection???
  ...
  -given by the number of of data points $K$, the number of parameters$\boldsymbol{\theta}$ to be estimated $|\boldsymbol{\theta}|$ and the maximumum likelihood $\hat{L}$ of the model to be estimated:
...
\mathcal{N}\left(\boldsymbol{\hat{f}}_{k}; \boldsymbol{\mu}_{m}, \boldsymbol{\Sigma}_{m}\right) = \frac{1}{(2\pi)^{\nicefrac{J^{*}}{2}} |\boldsymbol{\Sigma}_{m}|^{\nicefrac{1}{2}}} exp\lbrace -\frac{1}{2}\left(\boldsymbol{\hat{f}}_{k} - \boldsymbol{\mu}_{m})^{T}\boldsymbol{\Sigma}_{m}^{-1}(_{k} - \boldsymbol{\mu}_{m}\right) \rbrace
\end{equation}
...
\begin{algorithm}[h]
\caption{Expectation-Maximization for \acp{GMM} Algorithm}
\label{alg:chap10-gmm-em}
\begin{algorithmic}[1]
  \State Initialize an empty root node $\mathbb{N}_{r}$
\end{algorithmic}
\end{algorithm}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Mapping Features to MGXS Targets}
\label{subsec:chap10-feature-mapping}
