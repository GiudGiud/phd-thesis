...
for directory in directories:
    ...
    if 'statepoint.00220.h5' not in statepoints:
        statepoints.append('statepoint.00220.h5')
    ...
    for domain in mgxslib.domains:
        ...
        clusters = np.loadtxt(subdirectory + filename)
        ...
        print('STARTING LNS PANELIZATION')
        ...
        degen_df = degen_mgxs_pf.minor_xs('mean')
        degen_df = degen_df.transpose()
        degen_df.index -= sp.n_inactive
        degen_df.index *= 1e-1

        null_df = null_mgxs_pf.minor_xs('mean')
        null_df = null_df.transpose()
        null_df.index -= sp.n_inactive
        null_df.index *= 1e-1

        lns_df = lns_mgxs_pf.minor_xs('mean')
        lns_df = lns_df.transpose()
        lns_df.index -= sp.n_inactive
        lns_df.index *= 1e-1

        # DEGENERATE
        df = degen_df.pct_change().abs()
        mean_degen_df = df.mad(axis=1)
        max_degen_df = df.max(axis=1)

        # NULL
        null_df = null_df.pct_change().abs()

        # LNS
        lns_df = lns_df.pct_change().abs()
        mean_lns_df = lns_df.mad(axis=1)
        max_lns_df = lns_df.max(axis=1)

        # Make filename
        if directory in ['2x2', 'reflector', 'full-core']:
            if '1.6' in domain.fill.name:
                filename = '16-enr-dev-{}-{}.png'.format(mgxs_type, group)
            elif '2.4' in domain.fill.name:
                filename = '24-enr-dev-{}-{}.png'.format(mgxs_type, group)
            elif '3.1' in domain.fill.name:
                filename = '31-enr-dev-{}-{}.png'.format(mgxs_type, group)
        else:
            filename = '{}-dev-{}-{}.png'.format(directory.replace('.', ''), mgxs_type, group)
        ...
        # Pickle the Panels
        lns_mgxs_pf.to_pickle(subdirectory + filename.replace('.png', '.pkl'))
        degen_mgxs_pf.to_pickle(subdirectory + filename.replace('.png', '.pkl'))
        null_mgxs_pf.to_pickle(subdirectory + filename.replace('.png', '.pkl'))
        ...
        # FIXME: Line for NULL
        N = (sp.n_batches - sp.n_inactive - 20) / 200 #20.
        x0 = null_df.index.values[1]
        x1 = null_df.index.values[-1]
        y0 = null_df.iloc[1].values[0]
        y1 = 1. / np.sqrt(N) * y0
        print('NULL: (x0,y0) = ({},{}), (x1,y1) = ({},{})'.format(x0,y0,x1,y1))
#        plt.plot([x0, x1], [y0, y1], linestyle='-', color='g', linewidth=0.5)

        # FIXME: Line for DEGENERATE (MAX)
        y0 = max_degen_df.iloc[1]
        y1 = 1. / np.sqrt(N) * y0
        print('DEGENERATE (MAX): (x0,y0) = ({},{}), (x1,y1) = ({},{})'.format(x0,y0,x1,y1))
#        plt.plot([x0, x1], [y0, y1], linestyle='-', color='b', linewidth=0.5)

        # FIXME: Line for DEGENERATE (MEAN)
        y0 = mean_degen_df.iloc[1]
        y1 = 1. / np.sqrt(N) * y0
        print('DEGENERATE (MEAN): (x0,y0) = ({},{}), (x1,y1) = ({},{})'.format(x0,y0,x1,y1))

        # FIXME: Line for LNS (MAX)
        y0 = max_lns_df.iloc[1]
        y1 = 1. / np.sqrt(N) * y0
        print('LNS (MAX): (x0,y0) = ({},{}), (x1,y1) = ({},{})'.format(x0,y0,x1,y1))
#        plt.plot([x0, x1], [y0, y1], linestyle='-', color='r', linewidth=0.5)

        # FIXME: Line for LNS (MEAN)
        y0 = mean_lns_df.iloc[1]
        y1 = 1. / np.sqrt(N) * y0
        print('LNS (MEAN): (x0,y0) = ({},{}), (x1,y1) = ({},{})'.format(x0,y0,x1,y1))

        plt.legend(['Null', 'LNS (Max)', 'LNS (Mean)', 'Degenerate (Max)', 'Degenerate (Mean)'])
        plt.ylabel('Batchwise Change [%]', fontsize=16)
