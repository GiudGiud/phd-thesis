...
Estimates for the variance may be deduced from standard error propagation. Such analysis is widely discussed in the literature, and only a few of the key results necessary to estimate the variance for \ac{MGXS} will be presented here. The arithmetic combinations of interest for \ac{MGXS} generation include addition, subtraction, multiplication and division. 
...
%\begin{equation}
%\label{eqn:chap3-add}
%Z = X + Y \qquad\Rightarrow\qquad \sigma_{Z}^{2} = \sigma_{X}^{2} + \sigma_{Y}^{2} + 2\sigma_{XY}
%\end{equation}
...
%\begin{equation}
%\label{eqn:chap3-mult}
%Z = XY \qquad\Rightarrow\qquad \sigma_{Z}^{2} \approx Z^{2}\left[\left(\frac{\sigma_{X}}{X}\right)^{2} + \left(\frac{\sigma_{Y}}{Y}\right)^{2} + 2\frac{\sigma_{XY}}{Z}\right]
%\end{equation}

%\begin{equation}
%\label{eqn:chap3-div}
%Z = \frac{X}{Y} \qquad\Rightarrow\qquad \sigma_{Z}^{2} \approx Z^{2}\left[\left(\frac{\sigma_{X}}{X}\right)^{2} + \left(\frac{\sigma_{Y}}{Y}\right)^{2} - 2\frac{\sigma_{XY}}{Z}\right]
%\end{equation}
...
%\noindent where $\mathbb{E}$ is the expectation operator. The covariance is not generally computable using the standard formulation for a tally estimator in a Monte Carlo simulation. Although it would be possible to estimate the covariance using ensemble statistics\footnote{The covariance could be estimated from the results of an ensemble of independent \ac{MC} simulations.}, this is not generally feasible or practical. Instead, the covariance terms in Eqns.~\Crefrange{eqn:chap3-add}{eqn:chap3-div} are neglected, resulting in the following approximate forms for the variance:

%\begin{equation}
%\label{eqn:chap3-add-approx}
%Z = X + Y \qquad\Rightarrow\qquad \sigma_{Z}^{2} \approx \sigma_{X}^{2} + \sigma_{Y}^{2}
%\end{equation}

%\begin{equation}
%\label{eqn:chap3-sub-approx}
%Z = X - Y \qquad\Rightarrow\qquad \sigma_{Z}^{2} \approx \sigma_{X}^{2} + \sigma_{Y}^{2}
%\end{equation}

%\begin{equation}
%\label{eqn:chap3-mult-approx}
%Z = XY \qquad\Rightarrow\qquad \sigma_{Z}^{2} \approx Z^{2}\left[\left(\frac{\sigma_{X}}{X}\right)^{2} + \left(\frac{\sigma_{Y}}{Y}\right)^{2}\right]
%\end{equation}

%\begin{equation}
%\label{eqn:chap3-div-approx}
%Z = \frac{X}{Y} \qquad\Rightarrow\qquad \sigma_{Z}^{2} \approx Z^{2}\left[\left(\frac{\sigma_{X}}{X}\right)^{2} + \left(\frac{\sigma_{Y}}{Y}\right)^{2}\right]
%\end{equation}

%\noindent In general, the random variables for reaction rates and fluxes in the same volume of phase space are highly correlated, and neglecting the covariance leads to a poor approximation for the variance of \ac{MGXS}. However, it should be noted that division is the primary operation needed to combine tallies to compute \ac{MGXS}. Since the reaction rates and flux tallies have a positive correlation, the covariance term in Eqn.~\ref{eqn:chap3-div} reduces the estimate of the covariance. It follows that Eqn.~\ref{eqn:chap3-div-approx} generally provides a conservative estimate of the variance for \ac{MGXS} generated from \ac{MC} tallies.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Review of Previous Efforts}
\label{sec:chap3-lit-review}b
