...
first paragraph: objective, history
-MOC~\ref{askew1972moc}
-reference Annals paper~\cite{boyd2014openmoc} \\
-MG deterministic code
-uses MOC
-open source
-could have used any MG code
  -chose OpenMOC for familiarity, open source
-reference need for MG code to evaluate MGXS for this thesis

second paragraph: new features, intro following sections
-improvements to the Python API
-coupling to \texttt{openmc.mgxs}
...
-2D implementations
  -reference 3D MOC papers
-cyclic track wrapping

second paragraph: more detailed eqns/approx
-brief description of \ac{MOC} equations
-flat/linear source approx.
-isotropic approx.
-power source iteration scheme, w/ CMFD
-reference Annals paper~\cite{boyd2014openmoc} for more details \\
...
first paragraph: software design
-C/C++ with Python wrappers generated by SWIG (cite)
  -NVIDIA CUDA~\cite{nvidia2012cuda} for NVIDIA \ac{GPUs}
-object-oriented software design
-simulation output as HDF5
-see OpenMOC site, ANS paper
-include figure of SWIG~\cite{beazley2003swig}, C++, Python
-mention improvements to OO geometry to enable this thesis
-tie back to this thesis: easy to couple to OpenMC, OpenCG, ML libraries

\begin{figure}
  \centering
  \includegraphics[width=0.7\linewidth]{figures/workflow/openmoc/software-design}
\caption[the $k$-Nearest Neighbor centroid scheme for CMFD]{The programming model used in OpenMOC to couple compiled C/C++/CUDA code to the Python using \ac{SWIG}~\cite{boyd2014openmoc}.}
\label{fig:knn-cmfd}
\end{figure}
...
first paragraph: overview of \texttt{openmoc.materialize} module
-macros, nuclide-integrated
-types
-HDF5
-constant-in-angle 
-uses total/transport, nu-fission, (nu-)scattering matrix, chi
-tie back to this thesis work: used to automate piping of MGXS from \texttt{openmc.mgxs} 
...
\begin{figure}
  \centering
  \includegraphics[width=0.7\linewidth]{figures/workflow/openmoc/centroid}
